...
public class MapComponent extends JPanel implements TrafficSimObserver {
	...
	public void paintComponent(Graphics graphics) {
		...
		if (_map == null || _map.getJunctionList().size() == 0) {
	...
	private void drawRoads(Graphics g) {
		for (Road r : _map.getRoadList()) {
			...
			int x1 = r.getSrcJunc().getxCoor();
			int y1 = r.getSrcJunc().getyCoor();
			int x2 = r.getDestJunc().getxCoor();
			int y2 = r.getDestJunc().getyCoor();
	...
	private void drawVehicles(Graphics g) {
		for (Vehicle v : _map.getVehicleList()) {
